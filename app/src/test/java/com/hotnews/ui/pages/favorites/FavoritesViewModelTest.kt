...
class FavoritesViewModelTest {
    ...
    fun `initial state is loading`() {
        assertTrue(viewModel.uiState.value is FavoritesViewModel.State.Loading)
    }

    @Test
    fun `load data successfully`() = runTest {
        val testData = listOf(createTestFavorite())
        coEvery { mockRepository.allBooks } returns flow {
            emit(DBResult.Success(testData))
        }

        viewModel.getFavorites()
        advanceUntilIdle()

        assertTrue(viewModel.uiState.value is FavoritesViewModel.State.Data)
        val result = (viewModel.uiState.value as FavoritesViewModel.State.Data).data
        assertTrue(result is DBResult.Success)
        assertEquals(testData, (result as DBResult.Success).data)
    }
}